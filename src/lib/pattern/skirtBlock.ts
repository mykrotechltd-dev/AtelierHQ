import { MeasurementValues, MeasurementUnit, PatternResult, readMM, boundingBoxOfPoints } from "./geometry";
...
// pattern: it has no waist dart, no back-piece dart intake difference, and
// no seam allowance. Treat it as a starting shape to true up by hand.
...
export function generateSkirtBlock(
  ...
  const centerWaist: [number, number] = [0, 0];
  const sideWaist: [number, number] = [waistQuarter, 0];
  const sideHip: [number, number] = [hipQuarter, hipDepth];
  const sideHem: [number, number] = [hipQuarter, skirtLength];
  const centerHem: [number, number] = [0, skirtLength];
  ...
  warnings.push("Basic front block only: add a waist dart, seam allowance, and a separate back piece by hand before cutting fabric.");

  return {
    pieces: [
      {
        name: "Skirt — Front (basic block, no seam allowance)",
        pathMM: path,
        boundingBoxMM: boundingBoxOfPoints([centerWaist, sideWaist, sideHip, sideHem, centerHem]),
      },
    ],
    warnings,
  };
