...
export function generateBodiceBlock(
  ...
  const centerNeck: [number, number] = [0, 0];
  const shoulderPoint: [number, number] = [shoulderHalf, 0];
  const bustPoint: [number, number] = [bustQuarter, bustLevelY];
  const waistPoint: [number, number] = [waistQuarter, bodiceLength];
  const centerWaist: [number, number] = [0, bodiceLength];
  ...
  warnings.push(
    "Torso block only, straight lines: add armhole curve, neckline curve, shoulder slope, and bust dart by hand before cutting fabric."
  );

  return {
    pieces: [
      {
        name: "Bodice — Front (basic torso block, no seam allowance)",
        pathMM: path,
        boundingBoxMM: boundingBoxOfPoints([centerNeck, shoulderPoint, bustPoint, waistPoint, centerWaist]),
      },
    ],
    warnings,
  };
