...
const ALL_FIELDS: MeasField[] = [
  {
    key: "chest",
    label: "Chest / Bust",
    hint: "Full circumference at widest point",
  },
  { key: "waist", label: "Waist", hint: "Natural waistline circumference" },
  {
    key: "hips",
    label: "Hips",
    hint: "Full hip circumference at widest point",
  },
  {
    key: "shoulder",
    label: "Shoulder width",
    hint: "Nape to shoulder tip, both sides",
  },
  {
    key: "sleeveLength",
    label: "Sleeve length",
  ...
  { key: "neck", label: "Neck", hint: "Neck circumference (optional)" },
  {
    key: "thigh",
    label: "Thigh",
    hint: "Upper thigh circumference (optional)",
  },
  ...
  // estimated from the fields above when left blank (see bodice-calculator.ts).
  {
    key: "shoulderDrop",
    label: "Shoulder drop",
    hint: "Shoulder-top line down to the true shoulder tip (optional)",
  },
  {
    key: "bustDepth",
    label: "Bust depth",
    hint: "Neck point down to the bust apex (optional)",
  },
  {
    key: "centerFrontLength",
    label: "Centre front length",
    hint: "Neck pit to waist, straight down centre front (optional)",
  },
  {
    key: "acrossChestWidth",
    label: "Across chest",
    hint: "Full across-chest width at armhole depth (optional)",
  },
  {
    key: "centerBackLength",
    label: "Centre back length",
    hint: "Nape to waist, straight down centre back (optional)",
  },
  {
    key: "acrossBackWidth",
    label: "Across back",
    hint: "Full across-back width at armhole depth (optional)",
  },
  {
    key: "sideSeamLength",
    label: "Side seam length",
    hint: "Underarm to waist (optional, shared by front and back)",
  },
];
...
function SkirtThumbnail() {
  return (
    ...
    >
      <path
        d="M15 10 L65 10 L75 95 L5 95 Z"
        fill="#f5f0e8"
        stroke="#1c2850"
        strokeWidth="1.5"
      />
      ...
      />
      <polyline
        points="37,59 40,55 43,59"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
      <polyline
        points="37,81 40,85 43,81"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
...
function BodiceThumbnail() {
  return (
    ...
    >
      ...
      />
      <polyline
        points="37,22 40,18 43,22"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
      <polyline
        points="37,44 40,48 43,44"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
...
function SleeveThumbnail() {
  return (
    ...
    >
      ...
      />
      <polyline
        points="37,34 40,30 43,34"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
      <polyline
        points="37,86 40,90 43,86"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
...
function DressThumbnail() {
  return (
    ...
    >
      ...
      />
      <path
        d="M33 40 L30 50 L33 62 L36 50 Z"
        stroke="#1c2850"
        strokeWidth="1"
        strokeDasharray="4 2"
        fill="none"
      ...
      />
      <polyline
        points="39,28 42,24 45,28"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
      <polyline
        points="39,104 42,108 45,104"
        stroke="#b48c3c"
        strokeWidth="1"
        fill="none"
      />
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-4 py-3 mb-8">
        ...
        <p className="text-xs text-amber-800 dark:text-amber-300">
          <strong>Basic drafts only.</strong> These blocks are generated from
          standard quarter-measurement + ease formulas. Test in a toile and
          verify with a trained patternmaker before cutting final fabric. No
...
/** 1 centimetre in inches — measurement state below is stored natively in
 *  inches; writing each default/bound as `originalCmValue * CM` keeps the
 *  original cm value auditable next to its conversion. */
const CM = 1 / 2.54;
const DEFAULT_SKIRT_LENGTH = 60 * CM;
const DEFAULT_DRESS_LENGTH = 70 * CM;
const SKIRT_LENGTH_RANGE = { min: 30 * CM, max: 120 * CM };
const DRESS_LENGTH_RANGE = { min: 30 * CM, max: 130 * CM };
...
function DraftWorkspace({
  ...
  const [skirtLength, setSkirtLength] = useState(String(DEFAULT_SKIRT_LENGTH));
  const [dressLength, setDressLength] = useState(String(DEFAULT_DRESS_LENGTH));
  ...
  const [curveOverrides, setCurveOverrides] = useState<
    Record<string, Record<number, string>>
  >({});
  const [editingBlockId, setEditingBlockId] = useState<BlockType | null>(null);

  // Customer list
  const { results: customers, status: custStatus } = useCustomers(
    undefined,
    100,
  );
  ...
  const rawBlocks = useMemo(
    () =>
      generateAllBlocks(resolvedMeasurements, {
        skirtLength: parseFloat(skirtLength) || DEFAULT_SKIRT_LENGTH,
        dressLength: parseFloat(dressLength) || DEFAULT_DRESS_LENGTH,
        ease,
        bodiceShoulderDart,
        bodiceSwayback,
  ...
  const handleSaveCurveEdits = (
    blockId: BlockType,
    edits: { index: number; d: string }[],
  ) => {
  ...
  const handlePrefillFromCustomer = () => {
    ...
    (Object.keys(m) as (keyof typeof m)[]).forEach((k) => {
      if (k !== "notes" && m[k] !== undefined)
        prefilled[k as keyof Measurements] = String(m[k]);
    });
                  ...
                  {customerId &&
                    ...
                    !selectedCustomer.measurements && (
                      <p className="text-[11px] text-amber-600">
                        This customer has no saved measurements. Go to their
                        profile to add them.
                      </p>
                    )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {ukSize && (
                    <p className="text-[11px] text-muted-foreground">
                      Standard measurements for UK {ukSize}. These may not fit
                      accurately — for best results, enter bespoke measurements.
                    </p>
                  )}
                </div>
              )}
              ...
              {measSource === "bespoke" && (
                <div className="space-y-3 mt-1">
                  ...
                  {relevantFields.map((field) => {
                    ...
                    // `measurements` is always stored in inches — only the
                    // displayed/typed value is converted to the selected unit.
                    const storedIn = parseFloat(measurements[field.key] ?? "");
                    const displayValue = !isNaN(storedIn)
                      ? formatMeasurement(storedIn, unit)
                      : "";
                    return (
                      <div key={field.key}>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const inches = isNaN(typed)
                              ? undefined
                              : unitToIn(typed, unit);
                            setMeasurements((prev) => ({
                              ...prev,
                              [field.key]:
                                inches === undefined ? "" : String(inches),
                            }));
                          }}
                        />
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-1.5">
                  ...
                  {ease === "standard" &&
                    "Standard ease — comfortable everyday fit"}
                  {ease === "relaxed" &&
                    "Extra ease — loose, oversized silhouette"}
                </p>
              </div>

              {/* Skirt length (skirt only) — state stays in inches; display/input convert. */}
              {entry.id === "skirt" && (
                <div>
                  ...
                  <Input
                    type="number"
                    min={inToUnit(SKIRT_LENGTH_RANGE.min, unit)}
                    max={inToUnit(SKIRT_LENGTH_RANGE.max, unit)}
                    step="0.5"
                    className="text-sm"
                    value={formatMeasurement(
                      parseFloat(skirtLength) || DEFAULT_SKIRT_LENGTH,
                      unit,
                    )}
                    onChange={(e) => {
                      const typed = parseFloat(e.target.value);
                      setSkirtLength(
                        isNaN(typed)
                          ? String(DEFAULT_SKIRT_LENGTH)
                          : String(unitToIn(typed, unit)),
                      );
                    }}
                  />
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Waist to hem. Default{" "}
                    {formatMeasurement(DEFAULT_SKIRT_LENGTH, unit)}{" "}
                    {unitLabel(unit)} (midi).
                  </p>
                </div>
              )}

              {/* Dress skirt length (dress only) — same inches-stored/unit-displayed pattern. */}
              {entry.id === "dress" && (
                <div>
                  <Label className="text-xs font-medium mb-1 block">
                  ...
                  <Input
                    type="number"
                    min={inToUnit(DRESS_LENGTH_RANGE.min, unit)}
                    max={inToUnit(DRESS_LENGTH_RANGE.max, unit)}
                    step="0.5"
                    className="text-sm"
                    value={formatMeasurement(
                      parseFloat(dressLength) || DEFAULT_DRESS_LENGTH,
                      unit,
                    )}
                    onChange={(e) => {
                      const typed = parseFloat(e.target.value);
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Waist to hem. Default{" "}
                    {formatMeasurement(DEFAULT_DRESS_LENGTH, unit)}{" "}
                    {unitLabel(unit)} (below the knee).
                  </p>
                </div>
              )}
            ...
            <Card>
              <CardContent className="pt-4 pb-4">
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 gap-x-4 gap-y-2">
                  {ALL_FIELDS.filter(
                    (f) => resolvedMeasurements[f.key] !== undefined,
                  ).map((f) => (
                    <div key={f.key}>
                      ...
                      <p className="text-sm font-medium">
                        {formatMeasurement(resolvedMeasurements[f.key]!, unit)}{" "}
                        {unitLabel(unit)}
                      </p>
                    </div>
                  ))}
              ...
              >
                ...
                <div className="flex flex-col h-full overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 px-5 py-3 border-b">
                    ...
                    <div>
                      <p className="text-sm font-medium">
                        Edit curves — {block.name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Drag handles to reshape seam lines
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-hidden">
                    <CurveEditor
                      block={block}
                      onSave={(edits) =>
                        handleSaveCurveEdits(editingBlockId, edits)
                      }
