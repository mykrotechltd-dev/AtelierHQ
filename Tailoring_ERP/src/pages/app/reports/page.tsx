...
export default function ReportsPage() {
  ...
  const orderPieData = stats
    ? [
        { name: "Received", value: stats.orderCounts.received, key: "received" },
        { name: "In Progress", value: stats.orderCounts.in_progress, key: "in_progress" },
        { name: "Completed", value: stats.orderCounts.completed, key: "completed" },
        { name: "Delivered", value: stats.orderCounts.delivered, key: "delivered" },
      ].filter((d) => d.value > 0)
    : [];

  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      <PageHeader title="Reports" description="Business analytics and performance overview." />
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <SummaryCard label="Total Billed" value={fmt(stats.totalBilled, currency)} />
            <SummaryCard label="Total Collected" value={fmt(stats.totalCollected, currency)} />
            <SummaryCard
              label="Outstanding"
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1.5">
                    <span className="inline-block w-3 h-2 rounded bg-muted-foreground/25" /> Billed
                  </span>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Top Customers by Order Value</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => [fmt(Number(value), currency), "Total value"]}
                    />
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-muted-foreground text-xs uppercase tracking-wide">
                        ...
                        <th className="text-left py-2 pr-4 font-medium">Specialization</th>
                        <th className="text-right py-2 pr-4 font-medium">Tasks Done</th>
                        <th className="text-right py-2 pr-4 font-medium">In Progress</th>
                        <th className="text-right py-2 pr-4 font-medium">Pending</th>
                      ...
                      {workerPerf
                        ...
                        .map((w) => (
                          <tr key={w.name} className="border-b last:border-0">
                            ...
                            <td className="py-2 pr-4 text-right text-amber-500">{w.inProgress}</td>
                            <td className="py-2 pr-4 text-right text-muted-foreground">
                              {w.pending}
                            </td>
                            <td className="py-2 text-right">{fmt(w.payoutTotal, currency)}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
  ...
  return (
    <Card className={highlight ? "border-yellow-500/40" : ""}>
      <CardContent className="pt-5 pb-4">
        ...
        <p className={`text-2xl font-bold font-display mt-1 ${highlight ? "text-yellow-600" : ""}`}>
          {value}
        </p>
      </CardContent>
    </Card>
