...
function cBezierPoints(
  ...
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    const mt = 1 - t;
    const x = mt ** 3 * x0 + 3 * mt ** 2 * t * cp1x + 3 * mt * t ** 2 * cp2x + t ** 3 * x1;
    const y = mt ** 3 * y0 + 3 * mt ** 2 * t * cp1y + 3 * mt * t ** 2 * cp2y + t ** 3 * y1;
    pts.push([x, y]);
  }
  return pts;
}
...
function pathToPolyline(d: string): [number, number][] {
  ...
  for (const cmd of cmds) {
    if (cmd.t === "M") {
      ...
      const curve = cBezierPoints(cx, cy, cmd.cp1x, cmd.cp1y, cmd.cp2x, cmd.cp2y, cmd.x, cmd.y);
      pts.push(...curve.slice(1)); // skip first (already in pts)
      cx = cmd.x; cy = cmd.y;
    } else if (cmd.t === "Z") {
...
function drawPath(
  ...
  const relLines = pts.slice(1).map(([x, y], i) => [
    tx(x) - tx(pts[i][0]),
    ty(y) - ty(pts[i][1]),
  ]);
  ...
  if (path.type === "grainline" && pts.length >= 2) {
    ...
    doc.lines(
      [
        [tx(first[0]) - aw - tx(first[0]), ty(first[1]) + aw * 1.5 - ty(first[1])],
        [aw * 2, 0],
      ],
      tx(first[0]) - aw,
      ty(first[1]) + aw * 1.5,
...
function addBlockPage(doc: jsPDF, block: PatternBlock) {
  ...
  const vbWcm = block.viewBox.w; // block width in cm
  const vbHcm = block.viewBox.h; // block height in cm
  // 1 cm = 10 mm at 1:1; we need to scale to fit DRAW_W × DRAW_H
  const scaleX = DRAW_W / (vbWcm * 10); // mm/cm after scale
  const scaleY = DRAW_H / (vbHcm * 10);
  const fit = Math.min(scaleX, scaleY, 1); // never scale up past 1:1
  const mmPerCm = 10 * fit; // mm per cm in the drawing
  ...
  const drawnW = vbWcm * mmPerCm;
  const drawnH = vbHcm * mmPerCm;
  const ox = MARGIN + (DRAW_W - drawnW) / 2 - block.viewBox.x * mmPerCm;
  const oy = MARGIN + 20 + (DRAW_H - drawnH) / 2 - block.viewBox.y * mmPerCm;
  ...
  for (const path of block.paths) {
    drawPath(doc, path, ox, oy, mmPerCm);
  }

  // Labels
  doc.setLineDashPattern([], 0);
  for (const label of block.labels) {
    const lx = ox + label.x * mmPerCm;
    const ly = oy + label.y * mmPerCm;
    ...
    if (label.rotate) {
      doc.text(label.text, lx, ly, {
        angle: label.rotate,
        align: (label.anchor === "start" ? "left" : label.anchor === "middle" ? "center" : label.anchor === "end" ? "right" : "left") as "left" | "center" | "right",
      });
    } else {
      doc.text(label.text, lx, ly, {
        align: (label.anchor === "start" ? "left" : label.anchor === "middle" ? "center" : label.anchor === "end" ? "right" : "left") as "left" | "center" | "right",
  ...
  // Draw a scale bar: 10cm at current scale
  const barLen = 10 * mmPerCm;
  doc.setDrawColor(...NAVY);
  doc.setLineWidth(0.5);
  doc.setLineDashPattern([], 0);
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.text("10 cm", MARGIN + barLen / 2, sbY + 7, { align: "center" });
...
function addCoverPage(
  ...
  doc.setFontSize(12);
  doc.setTextColor(...GOLD);
  doc.text("AtelierHQ — Basic Draft Blocks", PAGE_W / 2, 33, { align: "center" });
  ...
  autoTable(doc, {
    ...
    body: blocks.map((b) => [
      b.name,
      Object.entries(b.labels.find((l) => l.text.includes("cm"))?.text ?? "")
        .join("")
        .trim() || "—",
  ...
  // Disclaimer
  const afterTable = ((doc as jsPDF & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? 140) + 15;
  ...
  doc.setFontSize(8);
  doc.setTextColor(...GRAY);
  doc.text(`Generated: ${new Date().toLocaleDateString()}`, PAGE_W - MARGIN, PAGE_H - MARGIN, {
    align: "right",
  });
}
...
export function generatePatternsPDF(blocks: PatternBlock[], customerName: string): jsPDF {
...
export function downloadPatternsPDF(blocks: PatternBlock[], customerName: string): void {
